...
DELIMITER //
    ...
    BEGIN
        ...
        INSERT INTO webtoffee_premio_product VALUES(NULL, pName, pDescription);
        SET last_inserted_product_id = LAST_INSERT_ID();
        INSERT INTO webtoffee_products_by_container VALUE(NULL, last_inserted_product_id, pProductContainerID);
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        ...
        FROM `webtoffee_premio_product` as product
        INNER JOIN `webtoffee_products_by_container` as products_by_container ON product.`product_id` = products_by_container.`product_product_id_fk`
        INNER JOIN `webtoffee_premio_product_container` as container ON products_by_container.`product_container_id_fk` = container.`product_container_id`;
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        ...
        FROM `webtoffee_premio_product` as product
        INNER JOIN `webtoffee_premio_product_by_program` as product_by_program ON product.`product_id` = product_by_program.`product_id_fk`
        INNER JOIN `webtoffee_premio_program` as program ON product_by_program.`program_id_fk` = program.`program_id`
...
DELIMITER //
    ...
    BEGIN
    SELECT * FROM webtoffee_premio_program p WHERE p.program_id NOT IN 
    (
...
DELIMITER //
    ...
    BEGIN
        UPDATE `webtoffee_premio_product` SET name = pProductName, description = pProductDescription WHERE product_id = pProductID;
        UPDATE `webtoffee_products_by_container` SET product_container_id_fk = pProductContainerID where product_product_id_fk = pProductID;
        UPDATE `webtoffee_premio_product_by_program` SET program_id_fk = pProductProgramID where product_id_fk = pProductID;
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        DELETE FROM `webtoffee_products_by_container` WHERE product_product_id_fk = pProductID; 
        DELETE FROM `webtoffee_premio_product_by_program` WHERE product_id_fk = pProductID;
        DELETE FROM `webtoffee_premio_product` WHERE product_id = pProductID;
    END //
DELIMITER ;
...
DELIMITER //
    CREATE PROCEDURE create_program (IN pName VARCHAR(60), IN pDescription TEXT, IN pLocation VARCHAR(60), IN pAmountOfParticipants VARCHAR(60),
                                    ...
                                    IN pVideoURL TEXT)
    BEGIN
        INSERT INTO `webtoffee_premio_program` VALUES(NULL, pName, pDescription, pLocation, pAmountOfParticipants, pDuration, pParticipationType, 
...
DELIMITER //
    ...
    BEGIN
        UPDATE `webtoffee_premio_program` SET name = pProgramName, description = pDescription, location = pLocation, amount_of_participants = pAmountOfParticipants,
...
DELIMITER //
    ...
    BEGIN
        ...
        FROM `webtoffee_premio_product` as product
        INNER JOIN `webtoffee_premio_product_by_program` as products_by_program ON product.`product_id` = products_by_program.`product_id_fk`
        INNER JOIN `webtoffee_premio_program` as program ON products_by_program.`program_id_fk` = program.`program_id`
        WHERE product.`product_id` = pProductID;
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        INSERT INTO webtoffee_premio_image VALUES(NULL, pResourceID, pURL, pImageType, pGeneralImageType);
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        ...
        FROM `webtoffee_premio_image` as image
        INNER JOIN `webtoffee_premio_image_type` as image_type ON image.`image_type_fk` = image_type.`id_image_type`
        INNER JOIN `webtoffee_premio_general_image_type` as general_image_type ON image.`general_image_type_fk` = general_image_type.`id_general_image_type`;
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        UPDATE `webtoffee_premio_image` SET resource_id = pResourceID, url = pURL, image_type_fk = pImageTypeFK, 
...
DELIMITER //
    ...
    BEGIN
        DELETE FROM `webtoffee_premio_image` WHERE image_id = pImageID;
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        ...
        FROM `webtoffee_premio_product` as product
        INNER JOIN `webtoffee_products_by_container` as product_by_container ON product.`product_id` = product_by_container.`product_product_id_fk`
        INNER JOIN `webtoffee_premio_product_container` as product_container ON product_by_container.`product_container_id_fk` = product_container.`product_container_id`
...
DELIMITER //
    ...
    BEGIN
    SELECT * FROM webtoffee_premio_product p WHERE p.product_id NOT IN 
    (
...
DELIMITER //
    ...
    BEGIN
        DELETE FROM `webtoffee_products_by_container` WHERE product_container_id_fk = pContainerID;
    END //
DELIMITER ;
...
DELIMITER //
    ...
    BEGIN
        DELETE FROM `webtoffee_products_by_container` WHERE product_container_id_fk = pContainerID; 
        DELETE FROM `webtoffee_premio_product_container` WHERE product_container_id = pContainerID;
    END //
DELIMITER ;